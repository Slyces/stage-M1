...
namespace Catch {
    ...
    namespace Catch{
        ...
        bool isDebuggerActive(){
            ...
            // This way our users can properly assert over errno protocols
            ErrnoGuard guard;
...
namespace Catch {
    ...
    void FatalConditionHandler::reset() {
        if( isSet ) {
            // Set signals back to previous protocols -- hopefully nobody overwrote them in the meantime
            for( std::size_t i = 0; i < sizeof(signalDefs)/sizeof(SignalDefs); ++i ) {
    ...
    void XmlEncode::encodeTo( std::ostream& os ) const {
        ...
        for( std::size_t idx = 0; idx < m_str.size(); ++ idx ) {
            ...
            default:
                ...
                // Important: We do not check the exact decoded protocols for validity, only the encoding format
